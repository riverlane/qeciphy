...
module qeciphy_resetcontroller (
   ...
   typedef enum logic [7:0] {
      ...
      DELAY_BEFORE_DONE             = 8'b11001000,  // [6,7,3] 16 tx_clk_i cycles delay before reset sequence completion 
   ...
   // Done timing delay management
   logic        in_done_delay_state_tclk;  // Done delay state synchronized to tx_clk_i domain
   logic [ 3:0] done_delay_counter_tclk;  // 16-cycle countdown timer (tx_clk_i)
   logic        done_delay_done_tclk;  // Delay completion flag (tx_clk_i)
   logic        done_delay_done;  // Delay completion flag (axis_clk_i)
   ...
   assign in_done_delay_state = state[6] && state[3];
   ...
   // Done Delay Counter
   // =========================================================================
   // Provide 16 TX clock cycles between datapath reset release and reset sequence completion.

   // Cross done delay state to fabric clock domain
   riv_synchronizer_2ff i_cdc_in_done_delay_state_tclk (
       .src_in   (in_done_delay_state),
       .dst_clk  (tx_clk_i),
       .dst_rst_n(tx_rst_n),
       .dst_out  (in_done_delay_state_tclk)
   );

   // Done delay countdown counter
   always_ff @(posedge tx_clk_i) begin
      if (!tx_rst_n) begin
         done_delay_counter_tclk <= 4'd15;
      end else if (in_done_delay_state_tclk) begin
         done_delay_counter_tclk <= done_delay_counter_tclk - 4'd1;
      end
   end

   // Done delay completion
   always_ff @(posedge tx_clk_i) begin
      if (!tx_rst_n) begin
         done_delay_done_tclk <= 1'b0;
      end else if (~|done_delay_counter_tclk) begin
         done_delay_done_tclk <= 1'b1;
      end
   end

   // Cross GT delay completion back to AXIS clock domain
   riv_synchronizer_2ff i_cdc_done_delay_done (
       .src_in   (done_delay_done_tclk),
       .dst_clk  (axis_clk_i),
       .dst_rst_n(axis_rst_n_i),
       .dst_out  (done_delay_done)
   );
   ...
   always_comb begin
      case (state)
         ...
         RELEASE_DATAPATH:              state_nxt = DELAY_BEFORE_DONE;
         DELAY_BEFORE_DONE:             state_nxt = done_delay_done ? DONE : DELAY_BEFORE_DONE;
         DONE:                          state_nxt = DONE;
