...
**Note for Altera profiles:** `render-design` skips IP generation — Altera IP cores are generated automatically during `make synth`. This step still generates the required `src/qeciphy_build_cfg_pkg.sv` and must be run before synthesis.
...
For Xilinx, add all files listed in `src_xilinx.f` and `xci.f` to your project. For Altera, add all files listed in `src_altera.f`; there is no `xci.f` for Altera designs.

#### Altera IP Generation (E-Tile / F-Tile)

Unlike Xilinx (where IP is pre-generated by `make render-design` as `.xci` files), Altera transceiver and clock IP is generated at synthesis time. There is no separate render step for IP.

**When using `make synth`:** IP generation is fully automatic. The build calls `qsys-script` for each applicable `.tcl` file in `vendors/altera/25.3.1/ip/`, passing the device part, device family, line rate (in Mbps), and reference clock frequency (in MHz) as parameters. The resulting `.ip` files are placed in `run/<project_name>/ip/` and added to the Quartus project automatically.

Variant exclusion applies: for `ETILE`, the scripts `qeciphy_ftile.tcl` and `refclk.tcl` are skipped; for `FTILE`, `qeciphy_etile.tcl` is skipped.

**When integrating into an external Quartus project:** Invoke `scripts/quartus_ip.tcl` directly via `quartus_sh`, passing the variant, device, family, project name, line rate in Mbps, reference clock frequency in MHz, and IP directory as arguments:

```bash
# Example for ETILE at 10.3125 Gbps with 156.25 MHz reference clock
quartus_sh --script scripts/quartus_ip.tcl -tclargs \
    "ETILE" "AGFB014R24B2E2V" "Agilex 7" "<your_project_name>" \
    "10312.5" "156.25" "vendors/altera/25.3.1/ip"
```

The script handles variant exclusion automatically, places the generated `.ip` files in `run/<your_project_name>/ip/`, and adds the corresponding `IP_FILE` assignments to the Quartus project.
